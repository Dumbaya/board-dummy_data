<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(isset($bg_user)==false){
			...
			if($gb_result==0){
				if ($bv->insert(4) && $bv->update(4)) {
					echo "<script>alert('게시판 추천 성공!');</script>";
					echo "<script>window.location.href='http://{도메인}/page/board/board_read.htm?page=1&bid=".$f_bid."&tmp_num=".$tmp_num_mb."';</script>";
				} else {
					echo "<script>alert('게시판 추천 실패!');</script>";
					echo "<script>window.history.back();</script>";
				}
			}
			else if($gb_result==1){
				if ($bv->delete(3) && $bv->update(5)) {
					echo "<script>alert('게시판 추천 취소 성공!');</script>";
					echo "<script>window.location.href='http://{도메인}/page/board/board_read.htm?page=1&bid=".$f_bid."&tmp_num=".$tmp_num_mb."';</script>";
				} else {
					echo "<script>alert('게시판 추천 취소 실패!');</script>";
					echo "<script>window.history.back();</script>";
				}
			}
		}
