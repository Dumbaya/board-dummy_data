<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if($cnt_row==0){
			if ($bv->insert(6) && $bv->update(10)) {
				echo "<script>alert('댓글 추천 성공!');</script>";
				echo "<script>window.location.href='http://localhost/page/board/board_read.htm?page=1&bid=".$f_bid."&tmp_num=".$tmp_num_mb."';</script>";
			} else {
				echo "<script>alert('댓글 추천 실패!');</script>";
				echo "<script>window.history.back();</script>";
			}
		}
		else if($cnt_row==1){
			if ($bv->delete(4) && $bv->update(11)) {
				echo "<script>alert('댓글 추천 취소 성공!');</script>";
				echo "<script>window.location.href='http://localhost/page/board/board_read.htm?page=1&bid=".$f_bid."&tmp_num=".$tmp_num_mb."';</script>";
			} else {
				echo "<script>alert('댓글 추천 취소 실패!');</script>";
				echo "<script>window.history.back();</script>";
			}
		}
	}
